...
PHP_MINIT_FUNCTION(tcpsniff)
{
    ...
    REGISTER_LONG_CONSTANT("TH_ECE", TH_ECE, CONST_CS | CONST_PERSISTENT);
    REGISTER_LONG_CONSTANT("TH_CWR", TH_CWR, CONST_CS | CONST_PERSISTENT);
    return SUCCESS;
}
                       ...
                       const u_char *payload, size_t payload_size)
{
    ...
    */
    ...
    // tcp_opt 已经转过字节序~
    add_assoc_long(&tcp_opt_z, "rcv_tsval", tcp_opt->rcv_tsval);
    add_assoc_long(&tcp_opt_z, "rcv_tsecr", tcp_opt->rcv_tsecr);
    add_assoc_long(&tcp_opt_z, "sack_ok", tcp_opt->sack_ok);
    add_assoc_long(&tcp_opt_z, "snd_wscale", tcp_opt->snd_wscale);
    add_assoc_long(&tcp_opt_z, "mss_clamp", tcp_opt->mss_clamp);
